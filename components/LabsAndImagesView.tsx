        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-dark-card rounded-lg shadow overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-dark-text-secondary">
                            ...
                            <tbody>
                                {labResults.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((lab) => (
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {imagingResults.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(img => (
                        <div key={img.id} className="bg-dark-card rounded-lg shadow overflow-hidden group">
                            <img src={img.thumbnailUrl} alt={img.studyName} className="w-full h-32 object-cover" />
                            <div className="p-4">
                                ...
                                <a href="#" className="text-xs text-accent-cyan mt-2 inline-block opacity-0 group-hover:opacity-100 transition-opacity">Ver reporte completo</a>
                            </div>
                        </div>
                    ))}
                </div>
